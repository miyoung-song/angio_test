...
void ChartCallout::paint(QPainter* painter, const QStyleOptionGraphicsItem* option, QWidget* widget)
{
    ...
    QPointF anchor = mapFromParent(m_chart->mapToPosition(m_anchor));
    QPointF ancho_r = (m_chart->mapToPosition(m_anchor));
    if (m_btoolTip)
    {
...
void ChartCallout::setText(const QString& text)
{
    ...
    m_textRect = metrics.boundingRect(QRect(0, 0, 50, 50), Qt::AlignLeft, m_text);
    m_textRect.translate(5, 5);
    prepareGeometryChange();
    m_textRect.setWidth(75);
...
{
    prepareGeometryChange();
    if (m_chart->mapToPosition(m_anchor).x() > 500)
    {
        setPos(m_chart->mapToPosition(m_anchor) + QPoint(-100, -50));
    }
    else {
